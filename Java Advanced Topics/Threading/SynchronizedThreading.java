...
class ATM {
	...
	// this synchronized keyword is used to avoid mismatching in threadig while multiple threading is running.

	static synchronized void withdraw(BankAccount bankAc, int amount, String Name) {
		
		int balance = bankAc.getBalance();
		
		if((balance - amount) < -bankAc.getOverdraft()){
		...
		System.out.println("Current Balance: " + bankAc.getBalance());
